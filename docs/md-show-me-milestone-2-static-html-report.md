...
이 milestone 이 만들어내는 HTML 본문에 *항상* 5룰을 적용한다. 자세한 정의와 근거는 PRD §"출력 본문 톤 차단 5룰". 요약만:

1. 영어 단어 음차 무분별 섞기 금지 — 일반어로 대체 가능하면 한글로.
2. 어려운 한자어 남발 금지 — "수행하다 / 활용하다" → "한다 / 쓴다".
3. 번역체 종결어미 금지 — "~할 수 있습니다 / ~되어집니다" → "~할 수 있어요 / ~돼요".
4. em dash (—) 한 단락당 1개 이내.
5. 지식의 저주 차단 — 전문 용어 첫 등장 시 한글 풀이 의무 (괄호 안 또는 hover).

박서윤 페르소나 (v1 primary, 비CS 풀스택 학습자) 의 "한글 사이 영어 단어 박혀 읽다 멈춤" pain 을 차단하는 직접 조치. 시니어 페르소나에게도 풀이는 hover / 괄호라 거슬리지 않음 (반복 시 생략).

## View 기본 layout — 베스트셀러 4단계 흐름 (PRD D16, 권장)

이 milestone 이 layout 을 LLM 자유에 맡기되, 베스트셀러 4단계 흐름을 *기본 권장 순서* 로 둔다.

```
1. 주의 끌기 (lead)       ← 한 줄 결론 또는 도발적 질문 (첫 viewport)
2. 소개 (premise)         ← 이 답이 왜 필요한지 1~2문장
3. 본문 (worked example)  ← 인용 카드 3~5개 (베이스라인 N1)
4. 적용 (transfer)        ← copy-as-prompt + 다음 질문 chip (베이스라인 N+4)
```

베이스라인 3 의 N1 (citation 카드) 과 N+4 (copy-as-prompt) 가 본문·적용 단계와 정렬되므로 추가 비용 거의 없음. **strict 강제 여부는 박서윤 dogfood 결과까지 보류** (PRD D16) — 짧은 lookup 질의에는 생략하는 게 자연스러울 수 있어 LLM 적합성 판단 여지를 남긴다.
...
- 본문 톤 차단 5룰 (PRD D15) 이 출력 HTML 본문에 적용된다 — em dash 단락당 1개 이내, 전문 용어 첫 등장 시 한글 풀이, 음차 / 한자어 / 번역체 절제.
- 베스트셀러 4단계 흐름 (PRD D16) 이 view 기본 layout 권장으로 살아 있다. strict 강제 여부는 박서윤 dogfood 결과까지 보류.
